...
impl SqliteMemoryStore {
    ...
    fn node_to_record(node: KnowledgeNode, embedding: Option<Vec<f32>>) -> crate::storage::memory_store::MemoryRecord {
    ...
    fn enforce_model(
        ...
        {
            let guard = self.registered_model.read().map_err(|_| {
                MemoryStoreError::Init("registered_model rwlock poisoned".into())
            })?;
        ...
        let stored: Option<ModelSignature> = writer.query_row(
            ...
            |row| {
                ...
                Ok(ModelSignature { name, dimension: dim as usize, hash })
            },
        ).optional().map_err(|e| MemoryStoreError::Backend(e.to_string()))?;
        drop(writer);
        if let Some(stored) = stored {
            ...
            // Populate cache
            let mut guard = self.registered_model.write().map_err(|_| {
                MemoryStoreError::Init("registered_model rwlock poisoned".into())
            })?;
            *guard = Some(stored);
        }
...
impl crate::storage::memory_store::LocalMemoryStore for SqliteMemoryStore {
    ...
    async fn health_check(&self) -> crate::storage::memory_store::MemoryStoreResult<crate::storage::memory_store::HealthStatus> {
    ...
    async fn registered_model(
        &self,
    ) -> crate::storage::memory_store::MemoryStoreResult<Option<crate::storage::memory_store::ModelSignature>> {
        ...
        let reader = self.reader.lock().map_err(|_| {
            MemoryStoreError::Init("Reader lock poisoned".into())
        })?;
    ...
    async fn insert(
        ...
        let domains_json = serde_json::to_string(&record.domains)
            .unwrap_or_else(|_| "[]".to_string());
        let scores_json = serde_json::to_string(&record.domain_scores)
            .unwrap_or_else(|_| "{}".to_string());
    ...
    async fn get(
        &self,
        id: uuid::Uuid,
    ) -> crate::storage::memory_store::MemoryStoreResult<Option<crate::storage::memory_store::MemoryRecord>> {
        use crate::storage::memory_store::MemoryStoreError;
        let node = self
        ...
        #[cfg(all(feature = "embeddings", feature = "vector-search"))]
        let embedding = self
            .get_node_embedding(&id.to_string())
            .ok()
            .flatten();
        #[cfg(not(all(feature = "embeddings", feature = "vector-search")))]
    ...
    ) -> crate::storage::memory_store::MemoryStoreResult<Vec<crate::storage::memory_store::SearchResult>> {
                ...
                let out = results
                    ...
                    .map(|r| {
                        let (domains, domain_scores) =
                            self.read_domain_columns(&r.node.id);
    ...
    async fn get_scheduling(
        &self,
        memory_id: uuid::Uuid,
    ) -> crate::storage::memory_store::MemoryStoreResult<Option<crate::storage::memory_store::SchedulingState>> {
    ...
    > {
        ...
        let nodes: Vec<KnowledgeNode> = stmt
            .query_map(rusqlite::params![before_str, limit as i64], Self::row_to_node)
            .map_err(|e| MemoryStoreError::Backend(e.to_string()))?
            .collect::<std::result::Result<Vec<_>, _>>()
            .map_err(|e| MemoryStoreError::Backend(e.to_string()))?;
        drop(stmt);
        ...
        let out = nodes
            ...
            .map(|node| {
                ...
                let id_uuid = uuid::Uuid::parse_str(&id_str).unwrap_or_else(|_| uuid::Uuid::new_v4());
    ...
    ) -> crate::storage::memory_store::MemoryStoreResult<Vec<crate::storage::memory_store::MemoryEdge>> {
    ...
    async fn get_neighbors(
        ...
        node_id: uuid::Uuid,
        depth: usize,
    ) -> crate::storage::memory_store::MemoryStoreResult<Vec<(crate::storage::memory_store::MemoryRecord, f64)>> {
        use crate::storage::memory_store::MemoryStoreError;
    ...
    async fn list_domains(
        ...
        let rows = stmt
            .query_map([], |row| {
                ...
                Ok((id, label, centroid_bytes, top_terms_json, memory_count, created_at_str))
            })
            .map_err(|e| MemoryStoreError::Backend(e.to_string()))?;
        let mut result = Vec::new();
        for row in rows {
            ...
            let top_terms: Vec<String> =
                serde_json::from_str(&top_terms_json).unwrap_or_default();
    ...
    async fn get_domain(
        ...
        let result: Option<(String, String, Option<Vec<u8>>, String, i64, String)> = reader
            ...
            .optional()
            .map_err(|e| MemoryStoreError::Backend(e.to_string()))?;
        let Some((id, label, centroid_bytes, top_terms_json, memory_count, created_at_str)) = result else {
            return Ok(None);
        };
        let centroid: Vec<f32> = centroid_bytes
            .map(|b| {
...
mod tests {
    ...
    fn trait_health_check_reports_healthy_on_fresh_db() {
        ...
        rt.block_on(async {
            ...
            assert!(matches!(h, crate::storage::memory_store::HealthStatus::Healthy));
        });
    }
        ...
        rt.block_on(async {
            for i in 0..3usize {
                ...
                let state = SchedulingState {
                    ...
                    next_review: Some(
                        chrono::Utc::now()
                            - chrono::Duration::days(3 - i as i64)
                    ),
                    reps: 1,
                    lapses: 0,
                };
            ...
            let due = s
                .get_due_memories(chrono::Utc::now(), 10)
                .await
                .unwrap();
            assert_eq!(due.len(), 3);
        });
